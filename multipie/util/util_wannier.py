...
# ==================================================
# Merge win and nnkp information
# ==================================================
...
def read_hr(topdir, seedname):
    ...
    def next_nonempty(f):
        ...
        for line in f:
            if line.strip():
                return line
        raise ValueError(f"unexpected end of {seedname}_hr.dat")

    with _open_text(Path(topdir) / f"{seedname}_hr.dat") as f:
        # The first line is a comment written by Wannier90.
        try:
            next(f)
        except StopIteration as exc:
            raise ValueError(f"invalid hr file: {seedname}_hr.dat") from exc
        ...
        if len(ndegen) != num_R or any(value <= 0 for value in ndegen):
            raise ValueError(f"invalid degeneracies in {seedname}_hr.dat")
        ...
        if any(line.strip() for line in f):
            raise ValueError(f"too many Hamiltonian entries in {seedname}_hr.dat")
...
def build_ket_wannier(nnkp, site_dict, rtol=1e-4, atol=1e-4):
    ...
    for iw, (pos_idx, l, m, r, s) in enumerate(zip(*arrays)):
        ...
        ket_wannier.append([name, sublattice, l, orbital])
